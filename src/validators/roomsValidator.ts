...
export const validateRoom = (roomToValidate: Room): string[] => {
    ...
    if(!Object.values(enumRoomType).includes(roomToValidate.room_type)){
    ...
    if(Array.isArray(roomToValidate.amenities)){
        const amenities = roomToValidate.amenities.map((amenity: enumAmenities) => {
            if(Object.values(enumAmenities).includes(amenity)){
                return true;
            }
            return false;
        });
    ...
    if(typeof roomToValidate.status !== "string" || !Object.values(enumRoomStatus).includes(roomToValidate.status)){
